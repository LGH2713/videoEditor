...
void Audio::sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while(len > 0) //输入参数len等于is->audio_hw_buf_size，是audio_open()中申请到的SDL音频缓冲区大小
    {
        if(is->audio_cp_index >= static_cast<int>(is->audio_frm_size)) {
            ...
            if(audio_size < 0) // 发生错误则保持静音
            {
                is->p_audio_frm = nullptr;
        ...
        // 用is->audio_cp_index标识重采样帧中已拷贝入SDL音频缓冲区的数据位置索引，len1表示本次拷贝的数据量
        len1 = is->audio_frm_size - is->audio_cp_index;
        if(len1 > len)
        {
            len1 = len;
        }
        // 2.将转换后的音频数据拷贝到音频缓冲区stream中，之后的播放就是音频设备驱动程序的工作
        if(is->p_audio_frm != nullptr)
        {
            memcpy(stream, static_cast<uint8_t *>(is->p_audio_frm) + is->audio_cp_index, len1);
        }
        else {
    ...
    if(!isnan(is->audio_clock)) {
        // 更新音频时钟，更新时刻：每次往声卡缓冲区考入数据后
        // 前面的audio_decode_frame中更新的is->audio_clock是以音频帧为单位，所以此处第二个参数要减去未拷贝数据量占用的时间
        PlayerClock::set_clock_at(&is->audio_clk,
                                  is->audio_clock - static_cast<double>(2 * is->audio_hw_buf_size + is->audio_write_buf_size) / is->audio_param_tgt.bytes_pre_sec,
                                  is->audio_clock_serial,
                                  audio_callback_time / 1000000.0);
    }
}

int Audio::audio_decode_frame(AVCodecContext *p_codec_ctx, PacketQueue *p_pkt_queue, AVFrame *frame)
{
    int ret;
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            if(ret >= 0) {
                // 时基转换，从d->avctx->pkt_time时基转换到1/frame->sample_rate时基
                ...
                std::cout << "audio avcodec successfully" << std::endl;

                return 1;
            }
            else if(ret == AVERROR_EOF)
            {
                av_log(nullptr, AV_LOG_INFO, "audio avcodec_receive_frame(): the decoder has been flushed\n");
                avcodec_flush_buffers(p_codec_ctx);
                return 0;
            }
            else if(ret == AVERROR(EAGAIN))
            {
                av_log(nullptr, AV_LOG_INFO, "audio avcodec_receive_frame(): input is not accepted in the current state\n");
                break;
            }
            else
            {
                av_log(nullptr, AV_LOG_ERROR, "audio avcodec_receive_frame(): other errors\n");
                continue;
            }
        }
        ...
        {
            // 2.将packet发送给解码器
            //   发送packet的顺序时按dts递增的顺序，入IPBBPBB
            //   pkt.pos变量可以标识当前packet在视频文件中的地址偏移
            if(avcodec_send_packet(p_codec_ctx, &pkt) == AVERROR(EAGAIN))
            {
                av_log(nullptr, AV_LOG_ERROR, "recrive_frame and send_packet both returned EAGAIN, which is an API violation.\n");
            }
...
int Audio::audio_decode_thread(void *arg)
{
    ...
    while(1) {
        ...
        if(got_frame)
        {
            ...
            // 更新音频frame队列大小及写指针
            FrameQueue::frame_queue_push(&is->audio_frm_queue);
        }
    }
}

int Audio::open_audio_stream(PlayerStat *is)
{
    ...
    if(p_codec == nullptr)
    {
        av_log(nullptr, AV_LOG_ERROR, "Cann't find codec!\n");
        return -1;
    }

    // 1.3构建解码器AVCodecContext
    // 1.3.1p_codec_ctx初始化：分配结构体，使用p_codec初始化相应成员为默认值
...
int Audio::audio_resample(PlayerStat *is, int64_t audio_callback_time)
{
    int data_size, resampled_data_size;
    AVChannelLayout dec_channel_layout;
    av_unused double audio_clock0;
    int wanted_nb_samples;
    Frame *af, *default_frame;

    //#if defined(_WIN32)
    //    while(FrameQueue::frame_queue_nb_remaining(&is->audio_frm_queue) == 0) {
    //        if((av_gettime_relative() - audio_callback_time) > 1000000LL * is->audio_hw_buf_size / is->audio_param_tgt.bytes_pre_sec / 2)
    //            return -1;
    //        av_usleep(1000);
    //    }
    //#endif
    ...
    // 若队列头部可读，则由af指向可读帧
    if(!(af = FrameQueue::frame_queue_peek_readable(&is->audio_frm_queue)))
        return -1;
    FrameQueue::frame_queue_next(&is->audio_frm_queue);

    // 根据frame中指定的音频参数获取缓冲区大小
    data_size = av_samples_get_buffer_size(nullptr,
                                           af->frame->ch_layout.nb_channels, // 声道数
                                           af->frame->nb_samples, // 本帧包含的单个声道的样本数
                                           static_cast<AVSampleFormat>(af->frame->format), // 需要手动转型 采样格式
                                           1); // 不对齐
    ...
    // is->audio_param_tgt 是SDL可接受的音频帧数，是audio_open()中取得的参数
    // 在audio_open()函数中又有”is->audio_src == is->audio_param_tgt“
    // 此处表示：如果frame中的音频参数 == is->audio_src == is->audio_param_tgt，那音频重采样的过程就免了（因此时is->swr_ctr是nullptr）
            ...
            af->frame->sample_rate != is->audio_param_src.freq)
    {
        ...
        if(!is->audio_swr_ctx || swr_init(is->audio_swr_ctx) < 0) {
            av_log(nullptr, AV_LOG_ERROR,
                   "Cann't create sample rate conversion of %d Hz %s %d channels to %d Hz %s %d channels!\n",
        ...
        is->audio_param_src.fmt = static_cast<AVSampleFormat>(af->frame->format);
    }
    ...
    if(is->audio_swr_ctx)
    {
        ...
        // 重采样输出参数：输出音频缓样本数（多加了256个样本）
        int out_count = (int64_t)wanted_nb_samples * is->audio_param_tgt.freq / af->frame->sample_rate + 256;

        // 重采样输出参数：输出音频缓冲区尺寸（以字节为单位）
        int out_size = av_samples_get_buffer_size(nullptr, is->audio_param_tgt.channels, out_count, is->audio_param_tgt.fmt, 0);
        int len2;
        if(out_size < 0)
        {
...
int Audio::open_audio_playing(void *arg)
{
    ...
    // 2.打开音频并创建音频处理线程
    // 2.1打开音频设备，获取SDL设备支持的音频参数actual_spec（期望的参数是wanted_spec，实际得到的是actual_spec）
    // 1）SDL提供两种使音频设备取得音频数据的方法
    ...
    // 2.2根据SDL音频参数构建音频重采样参数
    // wanted_spec是期望的参数，actual_spec是实际的参数，wanted_spec和actual_spec都是SDL中的参数
    // 此处audio_param是ffmpeg中的参数，此参数应保证是SDL播放支持的参数，后面重采样要用到此参数
    // 音频帧解码后得到的frame中的音频格式未必被SDL支持，比如frame可能是planar格式，但SDL2.0不支持
    // 若将解码后的frame直接送入SDL音频缓冲区，声音将无法正常播放。所以需要先将frame重采样（转换格式）为SDL支持的模式，然后再送入SDL音频缓冲区
    is->audio_param_tgt.fmt = AV_SAMPLE_FMT_S16;
    is->audio_param_tgt.freq = actual_spec.freq;
    av_channel_layout_default(&is->audio_param_tgt.channel_layout, actual_spec.channels);
//    std::cout << "channel: " << is->audio_param_tgt.channel_layout.nb_channels << std::endl;
    ...
    is->audio_hw_buf_size = actual_spec.size;
    is->audio_frm_size = 0;
    is->audio_cp_index = 0;

    // 3.暂停/继续音频回调处理，参数1表示暂停，0表示继续。
    //      打开音频设备后默认未启动回调处理，通过调用SDL_PauseAudio(0)来启动回调函数
    //      这样就可以在打开音频设备后先为回调函数安全舒适化数据，一切就绪后在启动音频回调
    //      在暂停期间，会将静音值往音频设备写
    SDL_PauseAudio(0);
